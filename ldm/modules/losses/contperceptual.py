...
class LPIPSWithDiscriminator3D(nn.Module):
    def __init__(self, disc_start, ae_start, logvar_init=0.0, kl_weight=1.0, pixelloss_weight=1.0,
                 disc_num_layers=3, disc_in_channels=3, disc_factor=1.0, disc_weight=1.0,
                 perceptual_weight=1.0, use_actnorm=False, disc_conditional=False,
                 disc_loss="hinge"):
    ...
    def forward(self, inputs, reconstructions, posteriors, optimizer_idx,
        ...
        inputs = rearrange(inputs, "(b f) c h w -> b c f h w", f = f)
        reconstructions = rearrange(reconstructions, "(b f) c h w -> b c f h w", f = f)
